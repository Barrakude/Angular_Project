...
export class TaskListService {
  ...
  getPageTasks(pagination?: IPaginationParams, filters?:ITaskFilters):Observable<IResponse<ITask[]>>{
    ...
    if(filters){
      if (filters.title) {
        // Usa 'title_like' per la ricerca parziale (case-insensitive)
        params = params.set('title_like', filters.title);
      }
      if (filters.description) {
        // Usa 'description_like' per la ricerca parziale
        params = params.set('description_like', filters.description);
      }
      if (filters.status) {
